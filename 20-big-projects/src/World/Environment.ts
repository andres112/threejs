...
export default class Environment {
  ...
  private setSunlight() {
    ...
    this.guiLights.add(this.sunlight, 'intensity', 0, 5, 0.01).name('Sunlight intensity');
  ...
  private setEnvironmentMap() {
    ...
    this.guiEnvMap.add(App.instance.scene,  'environmentIntensity').min(0).max(10).step(0.001).name('Intensity');
    this.guiEnvMap.add(App.instance.scene,  'backgroundIntensity').min(0).max(10).step(0.001).name('Background intensity');
  }
}
